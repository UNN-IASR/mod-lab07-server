...
namespace Lab07 {
    class Program {
        static void Main() {
            ...
            int requestsCount = 150;
            int streamsCount = 5;

            int clientIntensity = 15;
            int serverIntensity = 2 * streamsCount;

            const int second = 1000;
            int clientDelay = second / clientIntensity;
            int serverDelay = second / (serverIntensity / streamsCount);
            ...
            using (StreamWriter writer = new StreamWriter("../../../results.txt")) {
                double realClientIntensity = second / clientDelays.Average();
                double realServerIntensity = second / (server.serverDelays.Average() / streamsCount);

                writer.WriteLine("Всего: {0}", server.requestCount);
                writer.WriteLine("Обработано: {0}", server.processedCount);
                writer.WriteLine("Отклонено: {0}", server.rejectedCount);
                writer.WriteLine("\nОжидаемые результаты:");
                Print(clientIntensity, serverIntensity, streamsCount, writer);
                writer.WriteLine("\nФактические результаты:");
                Print(realClientIntensity, realServerIntensity, streamsCount, writer);
            }
        }
        private static void Print(double clientIntensity, double serverIntensity, int streamsCount, StreamWriter writer) {
